...
#Voici le prompt donné :
# Je veux que tu m’écrives un script Python complet d’évaluation offline pour mon système de recommandation de films.

# Contexte :
# - Mon fichier de ratings est `data/dataset/rating_for_testing.csv`
# - Mon fichier films est `data/dataset/movies_cleaned.csv`
# - Mon fichier de recommandations pré-calculées est `data/recommendation/movie_recommendations_combined.json`
# - Mon fichier Lambda réel est `amplify/functions/recommender/recommender.py`
# - La fonction à appeler dans la Lambda est `handler(event, context)`
# - Les notes sont sur 10
# - Un film est considéré comme aimé si `rating > 7.0`
# - Mon fichier ratings ne contient PAS de colonne `timestamp`
# - Le split train/test doit donc être aléatoire par utilisateur, avec `20%` des films aimés en test
# - Je veux un script sans argparse, avec les chemins définis en constantes en haut du fichier

# Je veux que le script :
# 1. charge les fichiers ratings et movies avec pandas
# 2. charge le JSON des recommandations pré-calculées
# 3. importe le vrai module `recommender.py` depuis `amplify/functions/recommender`
# 4. appelle la vraie Lambda via `recommender.handler(event, None)` au lieu d’utiliser une fonction locale de recommandation
# 5. mocke localement dans le script :
#    - `recommender.get_user_ratings`
#    - `recommender.load_recommendations_from_s3`
#    pour éviter d’appeler AWS pendant l’évaluation
# 6. convertisse les `movieId` en `str`
# 7. garde seulement les films présents à la fois dans ratings, movies et le JSON
# 8. évalue les recommandations avec :
#    - `precision@10`
#    - `recall@10`
#    - `ndcg@10`
#    - `catalog_coverage`
# 9. compare aussi les résultats à un baseline de popularité
# 10. affiche les résultats finaux proprement
# 11. contienne des commentaires clairs en français
# 12. soit exécutable tel quel

# Je veux aussi :
# - une fonction `dcg_at_k`
# - une fonction `ndcg_at_k`
# - une fonction `precision_at_k`
# - une fonction `recall_at_k`
# - une fonction `load_neighbors`
# - une fonction `call_lambda_handler_for_user`
# - une fonction `popularity_baseline`
# - une fonction `evaluate`
# - une fonction `interpret`

# Important :
# - le script doit être complet
# - il ne doit pas dépendre d’une colonne timestamp
# - il doit utiliser `RANDOM_STATE = 42`
# - il doit utiliser `TOP_K = 10`
# - il doit utiliser `LIKED_THRESHOLD = 7.0`

# Donne-moi uniquement le code final du script.
...
#Charge le fichier JSON contenant, pour chaque film, une liste de films recommandés.
...
#Permet une interprétation simple des résultats obtenus
